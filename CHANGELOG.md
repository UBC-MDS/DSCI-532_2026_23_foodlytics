...
The dashboard now loads data via parquet and DuckDB with filtering at the database level, supports strategic AI queries with a configurable focus, and has documented tests (Playwright for UI behavior, pytest for `filter_data`). Limitations include the need for a GitHub token for the AI tab and dataset size assumptions in tests. No intentional deviations from DSCI 531 visualization practices.

**Tests.** The unit tests (`test_filter_data`) cover the refactored `filter_data` function and edge cases: a single filter (one column), multiple filters (several columns combined), and no filter (full dataset returned). The Playwright tests ensure the dashboard shows the full dataset in its original state on load; that applying one or multiple filters produces the expected counts and table; and that the reset button restores the original dataset and value boxes. *What could break:* If `filter_data` or the reactive filtering logic changes, the unit tests would fail and filtered results could be wrong. If the initial state, filter behavior, or reset logic changes, the Playwright tests would fail—users could see inaccurate filtered data or lose the ability to return to the original state, and dashboard behavior would no longer match what we expect.
